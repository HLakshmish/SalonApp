...
function App() {
  ...
  const handleSalonEdit = (salon) => {
    ...
    setExistingLogoUrl(salon.logoUrl || null)
    setExistingBannerUrl(salon.bannerUrl || null)
    setExistingPhotosUrls(salon.photosUrls || [])
    setStatus(null)
    setError(null)
    setTimeout(() => {
      document.getElementById('salon-create-title')?.scrollIntoView({ behavior: 'smooth' })
    }, 50)
  }
  ...
  const renderSalonManagementPanel = () => {
    ...
    return (
      <>
        ...
        {/* Salon Hero Banner */}
        {selectedSalon.bannerUrl && (
          <div className="salon-detail-banner-wrapper">
            <img src={selectedSalon.bannerUrl} alt={selectedSalon.name} className="salon-detail-banner" />
          </div>
        )}

        {/* Salon Management / Details Panel */}
        <section className="panel" aria-labelledby="manage-salon-title">
          <div className="section-header salon-detail-header-row">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', width: '100%', flexWrap: 'wrap', gap: '1rem' }}>
              <div className="salon-header-logo-container">
                {selectedSalon.logoUrl ? (
                  <img src={selectedSalon.logoUrl} alt={`${selectedSalon.name} Logo`} className="salon-detail-logo" />
                ) : (
                  <SalonInitials name={selectedSalon.name} />
                )}
                <div>
              ...
              {user && Number(user.id) === Number(selectedSalon.ownerId) && (
                <div className="section-actions">
                  <button
                    ...
                    onClick={() => handleSalonEdit(selectedSalon)}
                  >
                    {t('editDetails')}
                  </button>
                </div>
            ...
            {selectedSalon.description && (
              <p className="salon-detail-description">{selectedSalon.description}</p>
            )}
          </div>

          {/* Photos Gallery */}
          {selectedSalon.photosUrls && selectedSalon.photosUrls.length > 0 && (
            <div className="salon-detail-gallery">
              {selectedSalon.photosUrls.map((url, idx) => (
                <div key={idx} className="gallery-image-wrapper">
                  <img src={url} alt={`${selectedSalon.name} gallery ${idx + 1}`} className="gallery-image" />
                </div>
              ))}
            </div>
          )}
                              ...
                              <article key={salon.id} className="salon-card">
                                ...
                                <div className="salon-card-inner">
                                  <div className="salon-card-header">
                                    {salon.logoUrl ? (
                                      <div className="salon-card-logo-wrapper">
                                        <img src={salon.logoUrl} alt={`${salon.name} Logo`} className="salon-card-logo" />
                                      </div>
                                    ) : (
                        ...
                        <label>
                          ...
                          {existingLogoUrl && (
                            <div className="image-preview-wrapper">
                              <img src={existingLogoUrl} alt="Current Logo" className="thumbnail-preview" />
                              <span className="preview-label">{t('currentLogo')}</span>
                            </div>
                          )}
                        </label>
                        <label>
                          ...
                          {existingBannerUrl && (
                            <div className="image-preview-wrapper">
                              <img src={existingBannerUrl} alt="Current Banner" className="thumbnail-preview banner-thumb" />
                              <span className="preview-label">{t('currentBanner')}</span>
                            </div>
                          )}
                        </label>
                        <label className="full-width">
                          ...
                          {existingPhotosUrls && existingPhotosUrls.length > 0 && (
                            <div className="photos-preview-wrapper">
                              <span className="preview-label">{t('currentPhotos')}</span>
                              <div className="photos-preview-grid">
                                {existingPhotosUrls.map((url, idx) => (
                                  <img key={idx} src={url} alt={`Photo ${idx + 1}`} className="thumbnail-preview photo-thumb" />
                                ))}
                              </div>
                            </div>
                          )}
