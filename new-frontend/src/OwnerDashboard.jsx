...
const OwnerDashboard = ({ setCurrentView, salons, setSalons, authToken, setAuthToken }) => {
  ...
  const fetchSeats = async () => {
    if (!salons[0]) return;
    try {
      const response = await fetch(`http://localhost:3000/api/salons/${salons[0].id}/seats`, {
  ...
  const fetchServices = async () => {
    if (!salons[0]) return;
    try {
      const response = await fetch(`http://localhost:3000/api/salons/${salons[0].id}/services`, {
        headers: { 'accept': 'application/json' }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchEmployees = async () => {
    if (!salons[0]) return;
    try {
      const response = await fetch(`http://localhost:3000/api/salons/${salons[0].id}/employees`, {
        headers: { 'accept': 'application/json' }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const handleUpdateSalon = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/salons/${salons[0].id}`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${authToken}` },
        body: formData
      });
  ...
  const handleAddSeat = async (e) => {
    ...
    if (!salons[0]) return;
    try {
      const res = await fetch(`http://localhost:3000/api/salons/${salons[0].id}/seats`, {
        method: 'POST',
  ...
  const handleAddService = async (e) => {
    ...
    if (!salons[0]) return;
    try {
      const res = await fetch(`http://localhost:3000/api/salons/${salons[0].id}/services`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authToken}` },
  ...
  const handleAddEmployee = async (e) => {
    ...
    if (!salons[0]) return;
    try {
      const res = await fetch(`http://localhost:3000/api/salons/${salons[0].id}/employees`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authToken}` },
    ...
    <div>
      ...
      <div className="dashboard-container">
        <div className="dashboard-header">
          ...
          {salons.length === 0 && !isCreatingSalon && (
        ...
        {isCreatingSalon && salons.length === 0 ? (
          ...
          <div style={{ maxWidth: '800px', margin: '0 auto' }}>
            <div className="salon-card">
              <div className="salon-logo-placeholder">
                {salons[0].logoUrl ? (
                  <img src={salons[0].logoUrl} alt="Logo" style={{width: '100%', height: '100%', objectFit: 'cover', borderRadius: '8px'}} />
                ) : (
                  <ImageIcon size={48} />
                )}
              </div>
              <div className="salon-info">
                <h2>{salons[0].name}</h2>
                <p><strong>Address:</strong> {salons[0].address}, {salons[0].city}, {salons[0].state} - {salons[0].pincode}</p>
                <p><strong>Phone:</strong> {salons[0].phoneNumber}</p>
                <p><strong>Description:</strong> {salons[0].description || 'No description provided.'}</p>
                
                <div style={{marginTop: '20px', display: 'flex', gap: '10px', flexWrap: 'wrap'}}>
                  <button className="outline-btn" onClick={() => setIsConfiguring(!isConfiguring)}>
                    {isConfiguring ? 'Close Configuration' : 'Manage Salon (Seats, Services, Employees, Appointments)'}
                  </button>
                  <button className="outline-btn" onClick={handleEditSalonClick}>
                    Edit Salon Details
                  </button>
                  <button className="outline-btn" style={{color: '#ff4d4d', borderColor: '#552222', backgroundColor: 'rgba(255, 77, 77, 0.1)'}} onClick={handleDeleteSalon}>
                    Delete Salon
                  </button>
                </div>
              </div>
